"""CPAM candidate readouts and historical ablations."""
...
from continual_core.validation import vector as _validate_vector


class CPAMStateMixin:
    """Expose candidate state without coupling shared evaluators to CPAM types."""
            ...
            if isinstance(value, np.ndarray) and id(value) not in seen:
                ...
                for index, item in enumerate(value):
                    if id(item) not in seen:
                        result[f"{name}_{index:04d}"] = item
                        seen.add(id(item))
        return result

    @property
    def persistent_arrays(self) -> tuple[np.ndarray, ...]:
        return tuple(self.persistent_state.values())
...
@dataclass
class CumulativeMemoryReadout(CPAMStateMixin):
...
@dataclass
class CumulativeMaturityReadout(CPAMStateMixin):
