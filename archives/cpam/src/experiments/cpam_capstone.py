"""CPAM capstone construction and candidate-specific analysis."""
...
from baselines.readouts import RLSReadout
...
from experiments.drift_suite import DriftSuiteConfig, transform_images
from experiments.legacy import evaluate_digits_locked
...
def run_model(
    ...
    *,
    ...
    learner = build_learner(name, config, seed)
    ...
    for phase_index, domain in enumerate(config.phase_domains):
        ...
        pre_evaluation = evaluate_digits_locked(
        ...
        seen.add(domain)
        evaluations = {
            evaluated_domain: evaluate_digits_locked(
                learner,
...
def phase_metrics(model: dict[str, Any]) -> dict[str, float]:
    phases = model["phases"]
    first_shifted = [
        phase
        for phase in phases
...
def run_memory_capstone(
    ...
    for seed in config.seeds:
        ...
        regimes = regime_splits(original, config, seed)
        ...
        runs.append(
            {
                "seed": seed,
                "models": {
                    name: run_model(name, config, regimes, seed=seed)
                    for name in names
                },
            }
        )
    ...
    metric_runs = [
        {name: phase_metrics(run["models"][name]) for name in names}
        for run in runs
    ]
    ...
    returns = {
        domain: {
            name: {
                metric: summary(
                    [
                        return_metrics(run["models"][name], domain)[metric]
                        for run in runs
                    ]
                )
...
# Historical aliases remain only for archived result-reproduction tests.
_build_learner = build_learner
_regime_splits = regime_splits
