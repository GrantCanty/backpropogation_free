"""Matched gradient controls for the recurring-domain experiment."""
...
from continual_core.datasets.digits import build_digits_segments, load_digits_split
...
from continual_core.spatial import PolarityConvolutionImageEncoder
    ...
    for architecture in BACKPROP_ARCHITECTURES:
        ...
        for rate in config.learning_rates:
            ...
            for seed in config.development_seeds:
                ...
                regimes = regime_splits(original, memory_config, seed)
...
def _run_test_seed(
    ...
    regimes = regime_splits(original, memory_config, seed)
    models = {
        name: run_model(name, memory_config, regimes, seed=seed)
    ...
    returns = {
        domain: {
            name: {
                ...
                for metric in return_metrics(runs[0]["models"][name], domain)
            }
            for name in names
        }
