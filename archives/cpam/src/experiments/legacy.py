"""Historical experiment runners retained during evaluator migration."""
...
from continual_core.state import locked_state
...
from continual_core.spatial import OnlineSpatialClassifier, SpatialClassifierConfig
from continual_core.streams import (
...
from methods.cpam.predictive import (
    ...
    if kind in (
        ...
        if kind == "managed16_predictive_conv":
            return OnlinePredictiveSpatialClassifier(
                ...
                readout,
                predictor_factory=lambda input_size, output_size, seed, regularization: RLSReadout(
                    input_size,
                    output_size,
                    seed=seed,
...
def digit_target(label: int) -> np.ndarray:
    target = np.zeros(10, dtype=np.float64)
    target[label] = 1.0
    return target


def process_digit_image(
    ...
    *,
    ...
    if getattr(learner, "observation_mode", "row_sequence") == "image":
...
def evaluate_digits(
    ...
    for index, (image, label) in enumerate(zip(images, labels)):
        target = digit_target(int(label))
        prediction = process_digit_image(learner, image, target=None)
...
def learner_training_arrays(learner: DigitsLearner) -> tuple[np.ndarray, ...]:
...
def evaluate_digits_locked(
    learner: DigitsLearner, images: np.ndarray, labels: np.ndarray
) -> dict[str, Any]:
    """Evaluate without feedback and verify that all learned arrays stay fixed."""

    with locked_state(learner):
        result = evaluate_digits(learner, images, labels)
        representation_diagnostics = getattr(
            learner, "representation_diagnostics", None
        )
        if callable(representation_diagnostics):
            result["representation_diagnostics"] = (
                representation_diagnostics(images)
            )
...
def run_digits_model(
    ...
    *,
    ...
    started = time.perf_counter()
    initial_evaluation = evaluate_digits_locked(
    ...
    for segment in segments:
        ...
        for index in segment.indices:
            ...
            target = digit_target(label)
            prediction = process_digit_image(
        ...
        started = time.perf_counter()
        evaluation = evaluate_digits_locked(
            learner, data.test_images, data.test_labels
        )
        evaluation_seconds += time.perf_counter() - started
        evaluation_history.append(
    ...
    learner_diagnostics = dict(getattr(learner, "diagnostics", {}))
    readout_diagnostics = dict(
        getattr(getattr(learner, "readout", None), "diagnostics", {})
    )
    if learner_diagnostics:
        result["diagnostics"] = learner_diagnostics
    if readout_diagnostics:
        result["readout_diagnostics"] = readout_diagnostics
    # Historical result aliases are selected by declared capabilities, not
    # concrete candidate classes. New experiments should consume the neutral
    # ``readout_diagnostics`` field.
    if "samples_in_slow_statistics" in readout_diagnostics:
        result["memory_diagnostics"] = readout_diagnostics
    if "samples_in_cumulative_statistics" in readout_diagnostics:
        result["maturity_diagnostics"] = readout_diagnostics
    if learner_diagnostics.get("jepa_inspired"):
        result["predictive_diagnostics"] = learner_diagnostics
    if getattr(learner, "observation_mode", "row_sequence") == "image":
...
# Compatibility aliases for historical callers. New experiment modules import
# the public names above.
_digit_target = digit_target
_process_digit_image = process_digit_image
_evaluate_digits = evaluate_digits
_learner_training_arrays = learner_training_arrays
_evaluate_digits_locked = evaluate_digits_locked
