"""A continuously active recurrent-reservoir baseline."""
...
from continual_core.protocols import FloatArray, OnlineReadout, ProtocolError
...
class OnlineReservoir:
    ...
    def persistent_state(self) -> dict[str, np.ndarray]:
        readout_arrays = self.readout.persistent_arrays
        return {
            "input_weights": self.input_weights,
            "recurrent_weights": self.recurrent_weights,
            "bias": self.bias,
            **{
                f"readout_{index:04d}": array
                for index, array in enumerate(readout_arrays)
            },
        }
