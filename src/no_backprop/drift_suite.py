"""Recurring image-domain shifts with one shared original training phase."""
...
def _run_model_suite(
    ...
    *,
    ...
    original_training = _train_phase(learner, original, seed=seed + 10)
    original_evaluation = _evaluate_digits_locked(
        learner, original.test_images, original.test_labels
    )
    zero_shot = {
        name: _evaluate_digits_locked(
    ...
    for transform_index, (name, shifted) in enumerate(transformed.items()):
        ...
        shifted_training = _train_phase(
            branch,
            shifted,
            seed=seed + 100 + transform_index,
        )
        original_after_shift = _evaluate_digits_locked(
            branch, original.test_images, original.test_labels
        )
        shifted_after_shift = _evaluate_digits_locked(
            branch, shifted.test_images, shifted.test_labels
        )
        return_training = _train_phase(
            branch,
            original,
            seed=seed + 200 + transform_index,
        )
        final_original = _evaluate_digits_locked(
            branch, original.test_images, original.test_labels
        )
        final_shifted = _evaluate_digits_locked(
            branch, shifted.test_images, shifted.test_labels
        )
